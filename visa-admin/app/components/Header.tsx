...
export function Header() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl border-b border-white/50 dark:border-gray-700/50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="#contacts"
              className="hidden sm:block bg-gradient-to-r from-green-500 via-pink-500 to-orange-500 text-white px-5 py-2.5 rounded-full font-bold text-sm hover:scale-105 transition shadow-lg"
            >
              {t('nav.applyVisa')}
            </a>
            ...
            <button
              ...
              className="md:hidden flex flex-col justify-center items-center w-10 h-10 rounded-lg bg-white/80 dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700"
      ...
      <div
        className={`fixed top-0 right-0 h-full w-72 bg-white dark:bg-gray-900 z-50 md:hidden transform transition-transform duration-300 ease-out shadow-2xl ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        ...
        <nav className="p-4 space-y-2">
          {navLinks.map(link => (
            <a
              ...
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-green-50 dark:hover:bg-green-900/30 hover:text-green-600 dark:hover:text-green-400 font-semibold transition"
            >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
          <a
            href="#contacts"
            onClick={handleLinkClick}
            className="block w-full text-center bg-gradient-to-r from-green-500 via-pink-500 to-orange-500 text-white px-5 py-3 rounded-xl font-bold hover:scale-[1.02] transition shadow-lg"
          >
            {t('nav.applyVisa')}
          </a>
        </div>
